...
export const equipmentLoansService = {
  async getAll(): Promise<EquipmentLoanDTO[]> {
    ...
    if (!response.ok) {
      throw new Error('Error al obtener los prestamos de equipo');
    }
  ...
  async create(data: CreateEquipmentLoanRequest): Promise<EquipmentLoanDTO> {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || 'Error al crear el préstamo de equipo');
    }
  ...
  async update(id: string, data: UpdateEquipmentLoanRequest): Promise<EquipmentLoanDTO> {
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.error || 'Error al actualizar el préstamo de equipo');
    }
